...
$unique_id = ! empty( $attributes['blockId'] ) ? $attributes['blockId'] : 'eelfg-icon-' . substr( md5( wp_json_encode( $attributes ) ), 0, 6 );
...
$el_classes = 'eelfg-icon-el eelfg-icon-view-' . $view;
if ( 'default' !== $view ) {
	$el_classes .= ' eelfg-icon-shape-' . $shape;
}

$block_wrap_attr = get_block_wrapper_attributes( array(
	'class' => 'eelfg-block eelfg-icon-block-wrap ' . $unique_id,
) );
if ( empty( $block_wrap_attr ) ) {
	$block_wrap_attr = 'class="eelfg-block eelfg-icon-block-wrap ' . esc_attr( $unique_id ) . '"';
}

// ---------------------------------------------------------------------------
// Inline styles (scoped to this block instance via $unique_id).
// ---------------------------------------------------------------------------
$selector     = '.eelfg-icon-block-wrap.' . $unique_id;
$style_handle = 'eelfg-icon-style';
...
$H::add_custom_style( $style_handle, $selector, '', [
	'.eelfg-icon-wrap'        => $H::get_inline_styles( $wrap_styles ),
	'.eelfg-icon-el'          => $H::get_inline_styles( $box_styles ),
	'.eelfg-icon-el i'        => $H::get_inline_styles( $i_styles ),
	'.eelfg-icon-el svg'      => $H::get_inline_styles( $svg_styles ),
	'.eelfg-icon-el:hover'    => $H::get_inline_styles( $box_hover ),
	'.eelfg-icon-el:hover i'  => $H::get_inline_styles( $i_hover ),
	'.eelfg-icon-el:hover svg' => $H::get_inline_styles( $svg_hover ),
] );
